...
export const Telemetry = {
  ...
  startRun(){
    this.current = {
      startedAt:Date.now(),
      dashAttempts:0, dashHits:0, nerfs:0,
      uniqueNerfed:new Set(), nerfsByCharacter:{}, multiplierAtNerf:[],
      firstNerfAt:null, lastNerfAt:null, chainLength:0, maxChainLength:0,
      teamContacts:0, rugContacts:0, chaseActivations:0,
      teamLifetimes:[], activeTeamsSamples:[], sampleTimer:0,
    };
  ...
  nerfEvent(characterId,multiplier,elapsedSeconds,lifetimeSeconds){
    ...
    c.dashHits++;c.nerfs++;c.uniqueNerfed.add(characterId);
    ...
    c.chainLength++;c.maxChainLength=Math.max(c.maxChainLength,c.chainLength);
  ...
  endRun(finalScore,survived,runStats={}){
    ...
    const summary={
      timestamp:new Date().toISOString(),score:finalScore,survived,
      nerfs:c.nerfs,uniqueTeamsNerfed:c.uniqueNerfed.size,
      fullRosterCompleted:c.uniqueNerfed.size>=6,
      dashAttempts:c.dashAttempts,dashHits:c.dashHits,
      dashHitRate:c.dashAttempts?+(c.dashHits/c.dashAttempts).toFixed(2):0,
      nerfsByCharacter:{...c.nerfsByCharacter},firstNerfAt:c.firstNerfAt,lastNerfAt:c.lastNerfAt,
      maxChainLength:c.maxChainLength,avgMultiplierAtNerf:+avg(c.multiplierAtNerf).toFixed(2),
      avgTeamLifetimeAtNerf:+avg(c.teamLifetimes).toFixed(1),teamContacts:c.teamContacts,rugContacts:c.rugContacts,
      chaseActivations:c.chaseActivations,avgActiveTeams:+avg(c.activeTeamsSamples).toFixed(1),matchDurationSec:+durationSec.toFixed(1),
      grazes:Number(runStats.grazes||0),pumps:Number(runStats.pumps||0),
      dashes:Number(runStats.dashes||c.dashAttempts||0),hits:Number(runStats.hits||0),bestMulti:Number(runStats.bestMulti||1),
    };
    this.history.push(summary);if(this.history.length>MAX_HISTORY)this.history.shift();
    this._persist();this._logSummary(summary);
    postServer({action:"end",sessionId:this.sessionId,summary},true);
    this.current=null;this.sessionId=null;return summary;
  },
  ...
  report(){
    ...
    const n=this.history.length,avg=key=>+(this.history.reduce((a,r)=>a+(r[key]||0),0)/n).toFixed(2);
    const counts=this.history.map(r=>r.nerfs).sort((a,b)=>a-b);
